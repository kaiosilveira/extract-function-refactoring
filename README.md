...
Extracting a block of code into a function is one of the most common refactorings out there. It helps giving more meaning to a given piece of code, as you'll be effectively naming a code block after its intent.
...
- Replace details code block with the new function:
...
The same mechanics applies to the code that records the due date:
...
- replace due date recording by the new function:
...
Things get trickier when it comes to extracting a function for the calculation of the `outstanding` amount, as it's being declared and then being incremented later on.
...
Finally, we can replace the first assignment to `outstanding` as being directly the result of a function call:
